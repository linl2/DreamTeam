...
const TeamMemberCard = ({ employee }) => {
  ...
  return (
    <div id={employee.id} onClick={handleEmployeeCardClick} style={{ cursor: "pointer" }} className={(employee.teamName === selectedTeam ? 'card m-2 standout' : 'card m-2')}>
      {(employee.picture !== "")? <img src={employee.picture} className="card-img-top" alt="profile" />  : (employee.gender === 'male') ? <img src={maleProfile} className="card-img-top" alt="profile" /> : <img src={femaleProfile} className="card-img-top" alt="profile" />}
